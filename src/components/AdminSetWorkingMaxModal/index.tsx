...
export interface AdminSetWorkingMaxModalProps {
  visible: boolean
  onClose: () => void
  onSuccess?: () => void
  userId: number
...
export function AdminSetWorkingMaxModal({
  visible,
  onClose,
  onSuccess,
  userId,
  ...
  currentValue,
  currentUnit = 'lb',
}: Readonly<AdminSetWorkingMaxModalProps>) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await adminService.setUserWorkingMax(userId, {
        exerciseId,
        value: num,
        unit,
      })
    ...
    >
      <div className="space-y-4">
        <Text variant="secondary" className="text-sm block">
          Set this user&apos;s working max for the exercise. Manual values are
          not overwritten by auto-calculation.
        </Text>
        <div>
          <label
            htmlFor="admin-wm-value"
